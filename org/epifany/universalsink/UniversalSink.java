...
package org.epifany.universalsink;
...
public final class UniversalSink {
	 ...
	 * @return 
	 */
	public static int compute( int[][] adjMatrix){
		int index = -1;
		int i = 1;	// i represents rows
		int j = 0;	// j represents columns
		int numVertices = adjMatrix.length;	// The number of vertices
		...
		while( i<numVertices && j<numVertices){
			...
			if( adjMatrix[i][j] == 0){
				if( i == j){/*Do Nothing*/}	// Does not provide enough information
		...
		return (numOutDegree == 0 && numInDegree == (numVertices-1)) ? index : -1;
	}
