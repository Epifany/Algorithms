...
public class UniversalSinkOld {
	...
	public int compute( int[][] adjMatrix){
	...
	private int computeAlgorithm(){
		...
		while( i<numVertices && j<numVertices){
			...
			if( adjacencyMatrix[i][j] == 1){
				...
				if( i == j){
					/*Do Nothing*/
				}	// matrix at [i][j] == 0 does not provide enough information, so do nothing
				else
					flags[i] = -1;
		...
		return (numRowDegree == adjacencyMatrix.length-1 && numColumnDegree == 0) ? index : -1;
	}
}